...
class FeatureSelectionEnv(gym.Env):
    """Custom Environment that follows gym interface and works with the feature selection task."""
    def __init__(self, model, criterion, data_loader, device):
        super(FeatureSelectionEnv, self).__init__()
        self.model = model
        self.criterion = criterion
        self.data_loader = data_loader  # train_iter
        self.device = device
        self.batch = None  # Store the current batch to be used across reset and step

        self.action_space = spaces.MultiBinary(192)
        # Observations are the feature vectors
        self.observation_space = spaces.Box(low=0, high=1, shape=(192,), dtype=np.float32)


    def reset(self):

        self.data_iterator = iter(self.data_loader)  # Reset iterator
        self.batch = next(self.data_iterator)  # Load the first batch
    ...
    def step(self, action):
        ...
        with torch.no_grad():
            # get a prediction used to compute acc, and then regard as reward
            outputs = self.model(text)
            _, predicted = torch.max(outputs.data, 1)
            correct = (predicted == labels).sum().item()
        # Calculate the reward based on accuracy
        reward = correct / labels.size(0)


        # Attempt to load the next batch for the next step
        try:
...
class ResNet1D(nn.Module):
    ...
    def __init__(self, in_channels, layers):
        ...
        # Adjust the initial in_channels after conv1 to match its output
...
class EnhancerClassifier_v2(nn.Module):
    def __init__(self, vocab_size, embedding_dim, device, num_classes=2, nhead=8, num_layers=3):
        ...
        # Initialize feature mask to ones (all features are used initially)
        self.feature_mask = torch.ones(192, dtype=torch.float32, device=device)
        # Initialize fused_features for batch processing
